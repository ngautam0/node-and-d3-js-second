(function mainFunction(){
  ...
  fileSystem.readFile(fileName,{encoding:"utf8"}, function(error,data){

    fileName="continent-lookup.csv";

    // read continent-country look up
    fileSystem.readFile(fileName,{encoding:"utf8"}, function(error,data1){
      ...
      var highPopulation=["India","China"];
      var highGdp=["USA","China","Japan"];
      ...
      splitHighLowPopulation(fileSystem,highPopulation);
     ...
     splitHighLowGdp(fileSystem,highGdp);

      // creating country continent-lookup
      // split each line individually by ,
        for (var i = 1; i < allTextLinesAsString.length; i++) {
          eachRow=allTextLinesAsString[i].split(",");
      // if its a country
          if(notCountry.indexOf(eachRow[0]) < 0){
            for (var z = 0; z < allContriesContinentList.length; z++) {
              var countryTempForMatching=allContriesContinentList[z].split(",");
              if (countryTempForMatching[0]==eachRow[0]) {
                countryContinent[eachRow[0]]=countryTempForMatching[1];
                break;
              }
...
//   // split each line individually by ,
  ...
  // data structure
   ...
   function splitHighLowPopulation(fileSystem,highPopulation)
   {
     ...
     fileSystem.readFile('./jsonfiles/graphDataPopulationOverPeriod.json',{encoding:"utf8"}, function(error,data){
   ...
   {
     fileSystem.readFile('./jsonfiles/graphDataGdpOverPeriod.json',{encoding:"utf8"}, function(error,data){
